  ...
  return `<section class="game-root game-killer-sudoku">
    ...
    <div class="cage-style-row" data-role="toggle-strip">
      ...
      <div class="toggle-group">
        ...
        <span class="cage-style-label">${t('game.killerSudoku.styleLines')}</span>
        <label class="switch"><input type="checkbox" data-action="cage-style"><span class="slider"></span></label>
        <span class="cage-style-label">${t('game.killerSudoku.styleColor')}</span>
      </div>
      <div class="toggle-group" title="${t('common.inputModeToggleTitle')}">
        <span class="toggle-name">${t('common.toggleInput')}</span>
        <span class="cage-style-label">${t('common.toggleOff')}</span>
        <label class="switch"><input type="checkbox" data-action="input-mode" checked><span class="slider"></span></label>
        <span class="cage-style-label">${t('common.toggleOn')}</span>
      </div>
    </div>
...
export function renderLevelsList(container, actions){
  ...
  if(saved){
    ...
    controls.querySelector('[data-action="discard"]').addEventListener('click', () => {
      if(confirm(t('common.discardConfirmNamed', { game: t('games.killerSudoku.title') }))){ clearSave(); renderLevelsList(container, actions); }
    });
